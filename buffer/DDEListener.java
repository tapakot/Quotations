...
class DDEListener implements DDEClientEventListener{
    ...
    /** when new data came */
    public void onItemChanged(String topic, String item, String data) { //item == R?C?
            int instrument = Integer.parseInt(item.substring(1, 2));
            if(instrument != buffer.realTimeThread.row){ return;}
            ...
            if (itemIndex == 2) { // 1/3 times
                ...
                buffer.bid = bid;
                buffer.ask = ask;
                if ((minute % 5 == 0) && (prevMinute != minute)) { //NULL_POINTER_EXCEPTION if starts at 00, 05
                    ...
                    if ((bid < minBid) || (minBid == 0)) {
                        minBid = bid;
                    }
                    if (ask > maxAsk) {
                        maxAsk = ask;
                    }
                    if ((ask < minAsk) || (minAsk == 0)) {
                        minAsk = ask;
                    }
                }
                if (buffer.test) {
